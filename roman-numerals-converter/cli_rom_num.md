...
- We should update dnf package and install AWS CLI v2. (for more information to install AWS CLI v2 please look at https://docs.aws.amazon.com/cli/latest/userguide/install-cliv2-linux.html)
```bash
sudo dnf update -y
...
```bash
aws ec2 describe-security-groups --group-names RN_SecGr
```
...
- This command is to run querry to get latest AMI ID (Linux 2023)
```bash
...
pip3 install flask
dnf install git -y
FOLDER="https://raw.githubusercontent.com/BabakTanriverdi/AWS_Projects/refs/heads/main/roman-numerals-converter/templates"
cd /home/ec2-user
wget -P templates ${FOLDER}/index.html
wget -P templates ${FOLDER}/result.html
wget https://raw.githubusercontent.com/BabakTanriverdi/AWS_Projects/refs/heads/main/roman-numerals-converter/roman-numerals-converter-app.py
python3 roman-numerals-converter-app.py
```

```bash
aws ec2 run-instances --image-id $LATEST_AMI --count 1 --instance-type t3.micro --key-name mod_1_key --security-groups RN_SecGr --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=babak_roman_numbers}]' --user-data file:///home/ec2-user/userdata.sh
or
...
aws ec2 run-instances \
    ...
    --key-name mod_1_key \
    --security-groups RN_SecGr \
    --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=babak_roman_numbers}]' \
    --user-data file:///home/babak/userdata.sh
```
...
```bash
aws ec2 describe-instances --filters "Name=tag:Name,Values=babak_roman_numbers" --query 'Reservations[].Instances[].PublicIpAddress[]'
...
```bash
aws ec2 delete-security-group --group-name RN_SecGr
```
